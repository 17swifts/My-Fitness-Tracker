...
import { Avatar, Box, Typography, CircularProgress, List, ListItem, Divider, IconButton, ListItemIcon } from '@mui/material';
...
const Profile = () => {
  ...
  const [workoutSummary, setWorkoutSummary] = useState({ completedWorkouts: 0, cardioActivities: 0 });
  ...
  useEffect(() => {
    ...
    const fetchWorkoutSummary = async (userId) => {
      try {
        ...
        const q = query(workoutsRef, where('userId', '==', userId), where('isComplete', '==', true));
      ...
      {userProfile && (
        <>
          ...
          <Box textAlign="left" mt={3} mb={3}>
            ...
            <Typography>Total Workouts Completed: {workoutSummary.completedWorkouts}</Typography>
            <Typography>Total Cardio Activities: {workoutSummary.cardioActivities}</Typography>
          </Box>
