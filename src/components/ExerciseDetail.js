...
const ExerciseDetail = () => {
  ...
  return (
    <Box sx={{ width: '100%', height: '100%', padding: 2, position: 'relative' }}>
      {/* Back Button */}
      <IconButton 
        onClick={() => navigate(-1)} 
      ...
      <Box sx={{ position: 'sticky', top: 0, backgroundColor: 'white', zIndex: 100, paddingBottom: 2 }}>
        <Grid container spacing={1}>
            ...
            <Grid item xs={9}>
                <Typography variant="h4" gutterBottom>{exerciseData.name}</Typography>
                <Typography variant="subtitle1" color="textSecondary" gutterBottom>{exerciseData.muscleGroup}</Typography>
                <Typography dangerouslySetInnerHTML={{ __html: exerciseData.description }}></Typography>
            </Grid>
        </Grid>
      </Box>
